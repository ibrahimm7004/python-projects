...
class Backtester:
    ...
    def backtest(self):
        ...
        for i, row in self.data.iterrows():
            ...
            # Check entry conditions for a new position
    ...
    def calculate_indicators(self):
        ...
        self.data['vwap_lowerband1'] = self.data['vwap'] - \
            self.data['vwap_stdev']
        self.data['vwap_upperband1'] = self.data['vwap'] + \
            self.data['vwap_stdev']
    ...
    def check_exit_conditions(self, row):
        ...
        if self.position == 'long':
            ...
            # Example TP price: Close price + 2 * ATR
            tp_price = row['Close'] + 2 * row['ATR']
            # Example SL price: Close price - 2 * ATR
            ...
            if row['High'] >= tp_price or row['Low'] <= sl_price:
                ...
                self.calculate_exit_statistics(
                    row)  # Calculate exit statistics
    ...
    def calculate_position_size(self, row):
        ...
        # Assuming 'Open' column contains the entry price
    ...
    def calculate_exit_statistics(self, row):
        ...
        holding_time = pd.to_datetime(
            exit_time) - pd.to_datetime(self.entry_time)
    ...
    def calculate_additional_metrics(self):
        ...
        self.stats['Buy and Hold Return (%)'] = (
            self.data.iloc[-1]['Close'] - self.data.iloc[0]['Close']) / self.data.iloc[0]['Close'] * 100
        ...
        if len(trades) > 0:
            self.stats['Win Rate (%)'] = sum(
                [1 for trade_return in trades if trade_return > 0]) / len(trades) * 100
        else:
        ...
        # Example negative returns as a list
        negative_returns = [-0.05, -0.03, -0.02, -0.01]
        downside_returns = np.minimum(
            negative_returns, 0)  # Extract negative returns
        # Calculate standard deviation of negative returns
        downside_deviation = np.std(downside_returns)
        self.stats['Sortino Ratio'] = (
            return_rate - risk_free_rate) / downside_deviation
        ...
        # Insert 0 as the first element for the first day
        daily_returns = np.insert(daily_returns, 0, 0)
        risk_adjusted_returns = daily_returns - risk_free_rate / \
            252  # Adjusting for daily risk-free rate
        self.stats['Sharpe Ratio'] = np.mean(
            risk_adjusted_returns) / np.std(risk_adjusted_returns)
        ...
        self.stats['Expectancy'] = (
            win_rate * average_win_size) - (loss_rate * average_loss_size)
        ...
        self.stats['SQN'] = np.mean(
            r_expectancy) / r_expectancy_std * np.sqrt(trades)
    ...
    def print_statistics(self):
        ...
        print("Avg Position Holding Time:",
              self.stats['Avg Position Holding Time'])
        print("Max Drawdown (% of capital):",
              "{:.2f}%".format(self.stats['Max Drawdown (%)']))
        print("SQN:", "{:.2f}".format(self.stats['SQN']))
        print("Start Date:", self.stats['Start Date'])
        print("End Date:", self.stats['End Date'])
        print("Capital Final ($):", "{:.2f}".format(
            self.stats['Capital Final ($)']))
        print("Buy and Hold Return (%):", "{:.2f}%".format(
            self.stats['Buy and Hold Return (%)']))
        ...
        print("Worst Trade (%):", "{:.2f}%".format(
            self.stats['Worst Trade (%)']))
        print("Best Trade (%):", "{:.2f}%".format(
            self.stats['Best Trade (%)']))
        print("Avg Trade Return (%):", "{:.2f}%".format(
            self.stats['Avg Trade Return (%)']))
    ...
    def plot_capital_chart(self):
        ...
        fig.update_layout(title='Backtest Capital Growth',
                          xaxis_title='Date', yaxis_title='Capital')
        fig.show()


# Usage
backtester = Backtester('data.csv', starting_capital=10000)
backtester.backtest()
