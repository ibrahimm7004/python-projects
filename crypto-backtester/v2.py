...
class backtester:
    ...
    def check_entry_conditions(self, row):
        open_time = pd.to_datetime(row[1]['open_time'], format='%Y-%m-%d %H:%M:%S')
        if row[1]['typical_price'] < row[1]['vwap_lowerband1']:
            if self.position != 'long':
                self.position = 'long'
                self.num_of_trades += 1
                self.entry_time = open_time
                self.long_timestamps.append(open_time)  # Append the long entry timestamp
    ...
    def calculate_transaction_fee(self, row):
        ...
        transaction_fee = self.position_size * row[1]['typical_price'] * fee_rate
    ...
    def plot_capital_chart(self):
        ...
        for row in self.data.iterrows():
            ...
            dates.append(pd.to_datetime(row[1]['open_time'], format='%Y-%m-%d %H:%M:%S'))
            # Calculate the percentage growth of capital over time and append to the list of capital percentages
            capital_growth = (row[1]['typical_price'] - self.starting_capital) / self.starting_capital * 100
            capital_percentage.append(capital_growth)

        fig = go.Figure(data=go.Scatter(x=dates, y=capital_percentage, mode='lines'))
        fig.update_layout(title='Capital Growth', xaxis_title='Time', yaxis_title='Capital Percentage')
        fig.show()
