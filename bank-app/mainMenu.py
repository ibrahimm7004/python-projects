...
def mainM():
    ...
    welcomeMessage = Label(mainMenu, text="Welcome User!",
                           wraplength=300, background="light green", font=com40)
    welcomeMessage.place(x=20, y=20)

    def showBalance():
        hiddenBalance.destroy()
        newLabel = Label(mainMenu, text="Rs. 65326", wraplength=300,
                         background='powder blue', font=('Helvetica Bold', 25))
        newLabel.place(x=305, y=95)

    com15 = Font(family="Comic Sans", size=15, weight="bold")
    displayBalance = Button(mainMenu, text="Display Balance", wraplength=300,
                            background="LemonChiffon2", font=com15, command=showBalance)
    displayBalance.place(x=300, y=40)

    hiddenBalance = Label(mainMenu, text="Rs. ******",
                          wraplength=300, font=('Helvetica Bold', 25))
    ...
    sendMoney = Button(mainMenu, text="Send Money", wraplength=300,
                       background="light yellow", font=com15, command=sendMoney)
    sendMoney.place(x=20, y=200)

    loadMoney = Button(mainMenu, text="Load Money", wraplength=300,
                       background="light pink", font=com15, command=ldMoney)
    ...
    card = Button(mainMenu, text="Order Card",
                  background="LightCyan2", font=com10, command=cardOrder)
    ...
    def chatButton():
        liveChat.bind(
            "<Button-1>", lambda r: callURLfunc("https://chat.meezanbank.com/Chat"))

    liveChat = Button(mainMenu, text="Live Chat",
                      background="LightCyan2", font=com10, command=chatButton)
