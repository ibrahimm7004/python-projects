...
welcomeWindow = Tk(className='Swift app - Welcome')  # welcomeWindow is an object of tkinter class
...
luc45 = Font(family="Lucida Console", size=25, weight="normal")  # defining a font to use
...
bankTitle = Label(welcomeWindow, text="Mobile Banking", font=('Lucida Console', 25, 'bold'),
                  wraplength=300)  # bankTitle is an object of class Label
bankTitle.place(x=50, y=100)  # Used to add a widget(Label) to a window

# Fonts:
mon25 = Font(family="Montserrat", size=25, weight="bold")
mon15 = Font(family="Montserrat", size=15, weight="bold")

exitButton = Button(welcomeWindow, background='light green', text="Exit", font=mon15, command=close)
LoginButton = Button(welcomeWindow, background='linen', text="Login", font=mon25, command=login)
SignupButton = Button(welcomeWindow, background='RosyBrown1', text="Sign Up", font=mon15,command=signup)
...
link = Label(welcomeWindow, text="About", font=('Helvetica bold', 15), fg="blue", cursor="hand2", underline=1)
